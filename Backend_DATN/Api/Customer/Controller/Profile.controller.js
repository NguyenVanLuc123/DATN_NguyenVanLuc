...
module.exports.postprofile = async (req, res) => {
  try {
    ...
    let img = req.body.img || ''; // Lấy URL từ req.body hoặc để trống
    let drivingLicense = req.body.drivingLicense || ''; // Lấy URL từ req.body hoặc để trống

    // 4) Nếu img hoặc drivingLicense không có trong req.body, lấy từ DB
    const customerData = await database.connectDatabase("SELECT img, driving_license FROM customer WHERE id = ?", [customerId]);
    if (customerData.length > 0) {
      if (!img) {
        img = customerData[0].img; // Lấy từ DB nếu không có trong req.body
      }
      if (!drivingLicense) {
        drivingLicense = customerData[0].driving_license; // Lấy từ DB nếu không có trong req.body
      }
    }
