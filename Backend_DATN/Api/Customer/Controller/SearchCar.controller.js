...
module.exports.getCarByid = async (req, res) => {
  ...
  try {
    ...
    if (decoded.is_owner === 0) {
      sql = `SELECT * FROM car WHERE status = 'available' and id =?`;
    }
    else {

      sql = `SELECT * FROM car WHERE  id =? and owner_id='${userResult[0].id}'`;
    }
    ...
    let results_additonal = [];
    let results_team_of_use = [];
    if (results) {

      results_additonal = await database.connectDatabase(sql_additonal, [id]);
