  ...
  const submit = () => {
    ...
    return (
        <div className="container mx-auto p-6">
            <nav className="text-sm mb-4"><Link to="/owner/cars" className="text-blue-600">My cars</Link> &gt; Add a car</nav>
                ...
                {['Basic', 'Details', 'Pricing', 'Finish'].map((t, i) => (
                    <button
                        ...
                        onClick={() => {
                            const target = i + 1;
                            if (target > step) {
                                // nếu click bước sau thì validate như Next
                                if (validateStep()) setStep(target);
                            } else {
                                // nếu click quay về bước trước thì không cần validate
                                setStep(target);
                            }
                        }}
            ...
            {step === 1 && (
                <div>
                    <div className="grid grid-cols-2 gap-6 bg-white p-6 rounded shadow">
                        ...
                        {/* Chỗ MFG */}
                        <div>
                            <label className="block mb-1">MFG*</label>
                            <input
                                type="date"
                                value={form.MFG}
                                ...
                                className="w-full border p-2 rounded"
                            />
                            {errors.MFG && <p className="text-red-500 text-sm">{errors.MFG}</p>}
                        </div>
                        <div>
                            ...
                            {errors.transmission_type && <p className="text-red-500 text-sm">Required</p>}
                        </div>
                        <div>
                            <label className="block mb-1">Fuel*</label>
                            <div className="flex space-x-2">
                            ...
                            {errors.fuel_type && <p className="text-red-500 text-sm">Required</p>}
                        </div>
                    </div>
                ...
                <div>
                    <div className="bg-white p-6 rounded shadow grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <input  min="1" type="number" value={form.mileage} onChange={handleInput('mileage')} className="w-full border p-2 rounded" />
                            {errors.mileage && <p className="text-red-500 text-sm">Required</p>}
                        </div>
                        <div>
                            <label className="block mb-1">Fuel consumption*</label>
                            <input min="1" type="number" value={form.fuel_consumption} onChange={handleInput('fuel_consumption')} className="w-full border p-2 rounded" />
                            {errors.fuel_consumption && <p className="text-red-500 text-sm">Required</p>}
                        </div>
                        <div>
                            <label className="block mb-1">Address*</label>
                            <select value={form.city} onChange={onProv} className="w-full border p-2 rounded">
                                <option value="">Province</option>
                                {provinces.map(p => <option key={p}>{p}</option>)}
                            </select>
                            {errors.city && <p className="text-red-500 text-sm">Required</p>}
                            <select value={form.district} onChange={onDist} className="mt-2 w-full border p-2 rounded">
                                <option value="">District</option>
                                {districts.map(d => <option key={d}>{d}</option>)}
                            </select>
                            {errors.district && <p className="text-red-500 text-sm">Required</p>}
                            <select value={form.ward} onChange={onWard} className="mt-2 w-full border p-2 rounded">
                        ...
                        <div className="col-span-2">
                            ...
                            <textarea value={form.description} onChange={handleInput('description')} rows={3} className="w-full border p-2 rounded" />
                        </div>

                        <div className="col-span-2">
                            <p className="mb-2 font-medium">Additional functions</p>
                        ...
                        <div className="col-span-2">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {['front_img', 'left_img', 'back_img', 'right_img'].map(f => (
                                    <div key={f}>
                                        <label className="block mb-1 capitalize">{f.replace('_', ' ').replace('_img', '')}</label>
                    ...
                    <div className="flex justify-between mt-6">
                        <button onClick={prev} className="text-gray-600 underline">Back</button>
                        <button onClick={next} className="bg-blue-600 text-white px-4 py-2 rounded">Next</button>
                    </div>
                </div>
            )}
            ...
            {step === 3 && (
                <div>
                    <div className="bg-white p-6 rounded shadow grid gap-6">
                        <div className="flex space-x-2 items-center">
                            <label className="w-40">Base price*</label>
                            <input min="1" type="number" value={form.base_price} onChange={handleInput('base_price')} className="border p-2 rounded w-full" />
                            <span>VND/Day</span>
                        </div>
                        {errors.base_price && <p className="text-red-500 text-sm">Required</p>}
                        <div className="flex space-x-2 items-center">
                            <label className="w-40">Required deposit*</label>
                            <input min="1" type="number" value={form.required_deposit} onChange={handleInput('required_deposit')} className="border p-2 rounded w-full" />
                            <span>VND</span>
                        </div>
                        {errors.required_deposit && <p className="text-red-500 text-sm">Required</p>}
            ...
            {step === 4 && (
                <div>
                    <p className="mb-4 font-medium">Preview</p>
                    <div className="bg-white p-6 rounded shadow mb-6">
                        ...
                        <div className="mt-4">
                            <h2 className="text-xl font-semibold mb-2">{form.brand} {form.model}</h2>
                    ...
                    <div className="flex justify-between">
                        <button onClick={prev} className="text-gray-600 underline">Back</button>
                        <button onClick={submit} disabled={loading} className="bg-blue-600 text-white px-6 py-2 rounded">{loading ? 'Submitting...' : 'Submit'}</button>
                    </div>
                </div>
            )}
        </div>
